...
pub fn is_string(vm: &mut VM, value: RantValue) -> RantStdResult {
    vm.cur_frame_mut()
        .write(value.get_type() == RantValueType::String);
    Ok(())
}

pub fn is_int(vm: &mut VM, value: RantValue) -> RantStdResult {
    vm.cur_frame_mut()
        .write(value.get_type() == RantValueType::Int);
    Ok(())
}

pub fn is_float(vm: &mut VM, value: RantValue) -> RantStdResult {
    vm.cur_frame_mut()
        .write(value.get_type() == RantValueType::Float);
    Ok(())
}
...
pub fn is_number(vm: &mut VM, value: RantValue) -> RantStdResult {
    vm.cur_frame_mut().write(matches!(
        value.get_type(),
        RantValueType::Int | RantValueType::Float
    ));
    Ok(())
}
...
pub fn is_bool(vm: &mut VM, value: RantValue) -> RantStdResult {
    vm.cur_frame_mut()
        .write(value.get_type() == RantValueType::Boolean);
    Ok(())
}
