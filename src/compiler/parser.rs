...
impl Op {
    ...
    fn is_kw_supported(kw_name: &str) -> bool {
        matches!(
            kw_name,
            KW_NEG | KW_NOT | KW_EQ | KW_NEQ | KW_GT | KW_GE | KW_LT | KW_LE
        )
    }
...
impl<'source, 'report, R: Reporter> RantParser<'source, 'report, R> {
    ...
    fn report_error(&mut self, problem: Problem, span: &Range<usize>) {
        ...
        self.reporter.report(CompilerMessage::new(
            problem,
            Severity::Error,
            Some(Position::new(line, col, span.clone())),
        ));
    }
    ...
    fn report_unexpected_token_error(&mut self, span: &Range<usize>) {
        self.report_error(
            Problem::UnexpectedToken(self.source[span.start..span.end].to_string()),
            span,
        )
    }
    ...
    fn parse_sequence_inner(
        ...
        // whitespace macro needs this, but https://github.com/rust-lang/rust/issues/15701
        ...
        (ignore both) => {{
          ...
          whitespace!(ignore next);
            ...
            macro_rules! inject_debug_info {
                () => {
                    if debug && _debug_inject_toggle {
                        let (line, col) = self.lookup.get(span.start);
                        sequence.push(Rc::new(Expression::DebugCursor(DebugInfo::Location {
                            line,
                            col,
                        })));
                    }
            ...
            macro_rules! emit_last_string {
                () => {{
                    inject_debug_info!();
                    sequence.push(Rc::new(Expression::Fragment(InternalString::from(
                        self.reader.last_token_string(),
                    ))));
                }};
            }
            ...
            macro_rules! consume_fragments {
                ($s:ident) => {
                    while let Some((token, span)) = self
                        .reader
                        .take_where(|t| matches!(t, Some((Escape(..) | Fragment, ..))))
                    {
                        match token {
                            Escape(ch) => match ch {
            ...
            match token {
                ...
                Keyword(KeywordInfo {
                    name: kw,
                    is_valid: is_kw_valid,
                }) if is_kw_valid && !Op::is_kw_supported(kw.as_str()) => {
                    let kwstr = kw.as_str();
                    match kwstr {
                        // Boolean constants
                        ...
                        KW_REQUIRE => no_flags!({
                            whitespace!(ignore both);
                            whitespace!(ignore next);
                            if let Some((require_first_arg_token, require_first_arg_token_span)) =
                                self.reader.next()
                            {
                                match require_first_arg_token {
                                    ...
                                    RantToken::Fragment => {
                                        ...
                                        if !is_valid_ident(require_alias.as_str()) {
                                            self.report_error(
                                                Problem::InvalidIdentifier(
                                                    require_alias.to_string(),
                                                ),
                                                &require_first_arg_token_span,
                                            );
                                        }
                                        whitespace!(ignore next);
                                        if !self
                                        ...
                                        if let Some((require_path_token, require_path_token_span)) =
                                            self.reader.next_solid()
                                        {
                                            ...
                                            match require_path_token {
                                                ...
                                                _ => self.report_error(
                                                    Problem::InvalidRequireArgumentToken,
                                                    &require_path_token_span,
                                                ),
                                            }
                                        } else {
                                            self.report_error(
                                                Problem::MissingRequireArgument,
                                                &self.reader.last_token_span(),
                                            );
                                        }
                                    }
                        ...
                        KW_RETURN | KW_CONTINUE | KW_BREAK | KW_WEIGHT | KW_ON => {
                            ...
                            let charm_sequence =
                                (!charm_sequence.is_empty()).then(|| Rc::new(charm_sequence));
                            match kw.as_str() {
                ...
                At => {
                    match self.reader.next_solid() {
                        Some((LeftBrace, _)) => {
                            // Read in the entire block
                            let parsed_block = self.parse_block(BlockParseMode::StartParsed(
                ...
                RightBracket => no_flags!({
                    match mode {
                        ...
                        SequenceParseMode::ParamDefaultValue => {
                            return Ok(ParsedSequence {
                ...
                Minus if is_seq_text || sequence.is_empty() => {
                    ...
                    if let Some((number_token, number_token_span)) = self.reader.take_where(|t| {
                        matches!(
                            t,
                            Some((
                ...
                token => {
                    ...
                    if let Some(mut next_op) = Op::from_token(&token) {
                        ...
                        loop {
                            ...
                            match next_op {
                                ...
                                Op::Prefix(prefix_op) => {
                                    ...
                                    if let Some(operand_next_lower_infix_op) =
                                        operand_next_lower_infix_op
                                    {
                        ...
                        match token {
                            ...
                            Keyword(kw) if !kw.is_valid => self
                                .report_error(Problem::InvalidKeyword(kw.name.to_string()), &span),
    ...
    fn parse_group_or_collection_init(
        &mut self,
        start_span: &Range<usize>,
    ) -> ParseResult<ParsedGroupOrCollection> {
        self.reader.skip_ws();
        ...
        if let Some((collection_type_token, _)) = collection_type_token_info {
            match collection_type_token {
                ...
                RantToken::Colon => {
                    ...
                    if self
                        .reader
                        .eat_where(|token| matches!(token, Some((RightParen, ..))))
                    {
                        ...
                        match seq_end {
                            ...
                            SequenceEndType::ProgramEnd => {
                                self.report_error(
                                    ...
                                    &super_range(start_span, &self.reader.last_token_span()),
                                );
                                return Err(());
                            }
                            _ => unreachable!(),
                ...
                RantToken::DoubleColon => {
                    ...
                    loop {
                        let key_expr = match self.reader.next_solid() {
                            ...
                            Some((LeftAngle, _)) => {
                                ...
                                if accessor_nodes.len() != 1 {
                                    self.report_error(
                                        Problem::InvalidShorthandVariable,
                                        &accessor_span,
                                    );
                                    continue;
                                }
                                ...
                                match accessor {
                                    Some(Expression::Get(getter)) => {
                                        ...
                                        if getter.path.is_variable_target() {
                                            if let Some(getter_var_name) =
                                                getter.path.var_name().as_ref().map(|v| v.as_str())
                                            {
                                                is_valid_shorthand = true;
                                                let key = InternalString::from(getter_var_name);
                                                let value_seq = Sequence::one(
                            ...
                            Some((StringLiteral(s), _)) => Some(MapKeyExpr::Static(s)),
                            // End of map
                        ...
                        if !self
                            .reader
                            .eat_where(|tok| matches!(tok, Some((Equals, ..))))
                        {
                            self.report_error(
                                Problem::ExpectedToken("=".to_owned()),
                                &self.reader.last_token_span(),
            ...
            loop {
                ...
                match seq_end {
                    ...
                    SequenceEndType::ProgramEnd => {
                        ...
                        if sequences.is_empty() {
                            self.report_error(
                                Problem::UnclosedParens,
                                &super_range(start_span, &self.reader.last_token_span()),
                            );
                        } else {
                            self.report_error(
                                Problem::UnclosedTuple,
                                &super_range(start_span, &self.reader.last_token_span()),
                            );
                        }
                        return Err(());
    ...
    fn parse_func_params(
        &mut self,
        start_span: &Range<usize>,
    ) -> ParseResult<Vec<ParsedParameter>> {
                    ...
                    match self.reader.next_solid() {
                        ...
                        Some((Fragment, span)) => {
                            ...
                            let (varity, full_param_span) =
                                if let Some((varity_token, varity_span)) = self
                                    .reader
                                    .take_where(|t| matches!(t, Some((Question | Star | Plus, _))))
                                {
                                    (
                                        match varity_token {
                                            // Optional parameter
                            ...
                            if is_sig_variadic && is_param_variadic {
                                ...
                                self.report_error(
                                    Problem::InvalidParamOrder(
                                        last_varity.to_string(),
                                        varity.to_string(),
                                    ),
                                    &full_param_span,
                                );
                                ...
                                Some((_, span)) => {
                                    self.report_error(
                                        Problem::UnexpectedToken(
                                            self.reader.last_token_string().to_string(),
                                        ),
                                        &span,
                                    );
                                    return Err(());
                        ...
                        Some((.., span)) => self.report_error(
                            Problem::InvalidIdentifier(self.reader.last_token_string().to_string()),
                            &span,
                        ),
                        None => {
    ...
    fn parse_func_access(&mut self) -> ParseResult<ParsedFunctionAccessor> {
        ...
        if let Some((func_access_type_token, func_access_type_span)) = self
            .reader
            .take_where(|t| matches!(t, Some((Dollar | Percent | Question, ..))))
        {
            match func_access_type_token {
                ...
                tt @ Dollar | tt @ Percent => {
                    ...
                    if is_const && !func_path.is_variable_target() {
                        self.report_warning(
                            ...
                            &func_access_type_span,
                        );
                    }

                    self.reader.skip_ws();

                    let ((body, params, end_func_sig_span, is_auto_hinted_def), captures) = self
                        .capture_pass(|self_| {
                            // Function params
                ...
                Question => {
                    ...
                    // Read function body
                    let (body, captures) =
                        self.capture_pass(|self_| self_.parse_func_body(&params))?;
              ...
              match arg_end {
                ...
                let old_pipeval = if is_piped {
                    ...
                    self.var_stack.define(
                        PIPE_VALUE_NAME.into(),
                        VarStats::new(VarRole::PipeValue, Default::default(), true),
                    );
                    old
                } else {
                    None
                ...
                if let Some((token, _)) = self.reader.next_solid() {
                    ...
                    self.track_variable_access(
                        &func_path,
                        false,
                        false,
                        &func_path_span,
                        Some(&mut is_auto_hinted),
                    );
    ...
    fn parse_access_mode(&mut self) -> VarAccessMode {
        if let Some((token, _span)) = self
            .reader
            .take_where(|t| matches!(t, Some((Slash | Caret, _))))
        {
            match token {
                // Accessor is explicit global
                Slash => VarAccessMode::ExplicitGlobal,
    ...
    fn parse_access_path(
    ...
    ) -> ParseResult<(AccessPath, Range<usize>)> {
        self.reader.skip_ws();
        let mut idparts = vec![];
        let start_span = self.reader.last_token_span();
        ...
        if let Ok(Some(_)) = self.try_read_signed_int() {
            ...
            if self
                .reader
                .eat_where(|t| matches!(t, Some((DoubleDot, ..))))
            {
                self.report_error(
                    Problem::AccessPathStartsWithSlice,
                    &super_range(&span, &self.reader.last_token_span()),
                );
            } else {
        ...
        loop {
            ...
            if self.reader.eat_where(|t| matches!(t, Some((Slash, ..)))) {
                    ...
                    Ok(Some(i)) => {
                            ...
                            .eat_where(|t| matches!(t, Some((DoubleDot, ..))))
                        {
                            ...
                            match self.try_read_signed_int() {
                                ...
                                Ok(Some(j)) => {
                                    idparts.push(AccessPathComponent::Slice(SliceExpr::Between(
                                        SliceIndex::Static(i),
                                        SliceIndex::Static(j),
                                    )));
                                }
                                Ok(None) => {
                                    match self.reader.peek() {
                                        ...
                                        Some((LeftParen, ..)) => {
                                            ...
                                            idparts.push(AccessPathComponent::Slice(
                                                SliceExpr::Between(
                                                    SliceIndex::Static(i),
                                                    SliceIndex::Dynamic(to_expr),
                                                ),
                                            ));
                                        }
                                        // From-slice with static from-bound
                                        Some((
                                            Slash | RightAngle | Equals | Question | Semicolon,
                                            ..,
                                        )) => {
                                            idparts.push(AccessPathComponent::Slice(
                                                SliceExpr::From(SliceIndex::Static(i)),
                                            ));
                                        }
                        ...
                        match self.reader.next() {
                            ...
                            Some((Fragment, span)) => {
                                ...
                                if is_valid_ident(varname.as_str()) {
                                    ...
                                    self.report_error(
                                        Problem::InvalidIdentifier(varname.to_string()),
                                        &span,
                                    );
                                }
                            }
                            ...
                            Some((DoubleDot, _)) => {
                                ...
                                match self.try_read_signed_int() {
                                    ...
                                    Ok(Some(to)) => {
                                        idparts.push(AccessPathComponent::Slice(SliceExpr::To(
                                            SliceIndex::Static(to),
                                        )));
                                    }
                                    Ok(None) => {
                                        match self.reader.peek() {
                                            ...
                                            Some((LeftParen, ..)) => {
                                                let to_expr =
                                                    Rc::new(self.parse_dynamic_key(true)?);
                                                idparts.push(AccessPathComponent::Slice(
                                                    SliceExpr::To(SliceIndex::Dynamic(to_expr)),
                                                ));
                                            }
                                            // Full-slice
                                            Some((
                                    ...
                                    match self.try_read_signed_int() {
                                        ...
                                        Ok(None) => {
                                            match self.reader.peek() {
                                                ...
                                                Some((
                                                    Slash | RightAngle | Equals | Question
                                                    | Semicolon,
                                                    ..,
                                                )) => {
                                                    idparts.push(AccessPathComponent::Slice(
                                                        SliceExpr::From(SliceIndex::Dynamic(expr)),
                                                    ));
                                                }
                                                // Found something weird as the to-bound, emit an error
                                                Some(_) => {
                ...
                return Ok((
                    AccessPath::new(idparts, access_mode),
                    start_span.start..self.reader.last_token_span().start,
                ));
            }
        }
    }
    ...
    fn parse_dynamic_key(&mut self, expect_opening_paren: bool) -> ParseResult<Sequence> {
        ...
        let ParsedSequence {
            sequence, end_type, ..
        } = self.parse_sequence(SequenceParseMode::DynamicKey)?;
    ...
    fn capture_pass<T>(
        &mut self,
        parse_func: impl FnOnce(&mut Self) -> ParseResult<T>,
    ) -> ParseResult<(T, Vec<Identifier>)> {
        ...
        self.capture_stack
            .push((capture_height, Default::default()));
        ...
        loop {
            ...
            let modifier = if self.reader.eat_kw(KW_EDIT) {
                ...
                let input_id = if self.reader.eat(RantToken::Colon) {
                    ...
                    self.var_stack.define(
                        input_id.clone(),
                        VarStats::new(VarRole::Normal, input_id_span, true),
                    );
                    Some(input_id)
                };
                self.reader.skip_ws();
    ...
    fn track_variable(
        ...
        let (prev_tracker, requested_depth, found_depth) = match access_kind {
            VarAccessMode::Local => (self.var_stack.get(id), 0, self.var_stack.depth_of(id)),
        ...
        match access_kind {
            ...
            VarAccessMode::ExplicitGlobal => {
                self.var_stack
                    .define_parent(id.clone(), v, self.var_stack.depth());
            }
        }
    }

    #[inline]
    fn track_variable_access(
        &mut self,
        path: &AccessPath,
        is_write: bool,
        has_fallback: bool,
        span: &Range<usize>,
        out_auto_hinted_read: Option<&mut bool>,
    ) {
        ...
        if let Some(id) = &path.var_name() {
            let tracker = match path.mode() {
                VarAccessMode::Local => self.var_stack.get_mut(id),
                VarAccessMode::Descope(n) => self.var_stack.get_parent_mut(id, n),
                VarAccessMode::ExplicitGlobal => {
            ...
            if let Some(tracker) = tracker {
                if is_write {
                    ...
                    if tracker.has_fallible_read
                        && tracker.role == VarRole::FallibleOptionalArgument
                    {
    ...
    fn analyze_top_vars(&mut self) {
        ...
        for (name, role, span) in unused_vars {
            match role {
                ...
                VarRole::Argument | VarRole::FallibleOptionalArgument => {
                    self.report_warning(Problem::UnusedParameter(name), &span)
                }
    ...
    fn parse_accessor(&mut self) -> ParseResult<ParsedAccessor> {
        ...
        'read: loop {
            ...
            if !accessors.is_empty()
                && self
                    .reader
                    .eat_where(|t| matches!(t, Some((RightAngle, ..))))
            {
                break;
            }

            let (is_def, is_const_def) = if let Some((def_token, ..)) = self
                .reader
                .take_where(|t| matches!(t, Some((Dollar | Percent, ..))))
            {
                match def_token {
                    // Variable declaration
                    Dollar => (true, false),
                ...
                if let Some((token, _token_span)) = self.reader.next_solid() {
                    match token {
                        ...
                        Equals => {
                            ...
                            let def_span =
                                access_start_span.start..self.reader.last_token_span().start;
                            self.track_variable(
                                &var_name,
                                &access_mode,
                                is_const_def,
                    ...
                    match token {
                        ...
                        RightAngle => {
                            self.track_variable_access(
                                ...
                                false,
                                &var_path_span,
                                Some(&mut is_auto_hinted_get),
                            );
                            is_auto_hinted_accessor |= is_auto_hinted_get;
                            add_accessor!(Expression::Get(Getter {
                                path: Rc::new(var_path),
                                fallback: None
                            }));
                            break 'read;
                        }
                        ...
                        Question => {
                            ...
                            add_accessor!(Expression::Get(Getter {
                                path: Rc::new(var_path),
                                fallback: Some(Rc::new(fallback_expr))
                            }));
                        ...
                        comp_op @ (PlusEquals | MinusEquals | StarEquals | SlashEquals
                        | DoubleStarEquals | PercentEquals | AndEquals
                        | VertBarEquals) => {
                            self.reader.skip_ws();
                            ...
                            let op_lhs = Rc::new(Sequence::one(
                                Expression::Get(Getter {
                                    path: Rc::new(var_path.clone().add_descope(1)),
                                    fallback: None,
                                }),
                                &self.info,
                            ));
            ...
            match token {
                ...
                RantToken::Minus => {
                        ...
                        .take_where(|t| matches!(t, Some((RantToken::IntegerPositive(_), _))))
                    {
                        return match nt {
                            PositiveIntegerToken::Value(n) => {
                                match self.try_sign_unsigned_int(n, true, &span) {
                                    Ok(i) => Ok(Some(i)),
                                    Err(()) => Err(()),
                                }
                            }
